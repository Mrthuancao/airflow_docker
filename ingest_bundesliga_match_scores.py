...
def get_key_type(ti, **kwargs):
    ...
    metadata_table_name = 'budesliga_match_scores'
    metadata_key = kwargs['metadata_key']
    
    dataset_type_select_sql = f"SELECT m_key_type FROM {metadata_table_name} WHERE m_key = '{metadata_key}'"
    
    cur.execute(dataset_type_select_sql)
    s3_dataset_type = cur.fetchone()[0]
    ...
    get_key_type_from_postgre = PythonOperator(
        task_id="get_key_type_from_postgre",
    ...
    file_sniffer_task = S3KeySensor(
        ...
        bucket_name='airflow',
        bucket_key='bundesliga_match_scores.csv',
        wildcard_match=True,
